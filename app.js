...
const NAMA_STORE = vars.NAMA_STORE || '@aryusZIVPN';
...
// ================== 🔐 FORCE JOIN ==================
const REQUIRED_GROUP = GROUP_ID; // pakai dari config kamu

async function isUserInGroup(ctx) {
  try {
    const member = await ctx.telegram.getChatMember(REQUIRED_GROUP, ctx.from.id);
...
bot.use(async (ctx, next) => {
  ...
  // biar /start tetap bisa
  if (ctx.message && ctx.message.text === "/start") {
    return next();
  }

  const isJoined = await isUserInGroup(ctx);
  ...
  if (!isJoined) {
    return ctx.reply(
      `❌ Kamu harus join grup dulu!\n\nKlik tombol di bawah ini 👇`,
      {
        reply_markup: {
          inline_keyboard: [
            [{ text: "🔗 Join Grup", url: "https://t.me/ZIVPNbyARYUS" }]
          ]
        }
      }
    );
...
👨‍💻 <b>Pembuat:</b> @aryusZIVPN
🛠️ <b>Credit:</b> aryusZIVPN × API POTATO
...
bot.action('service_trial2', async (ctx) => {
  try {
    ...
    await ctx.reply(
      ...
      `👤 Admin: <a href="https://t.me/aryusZIVPN">@aryusZIVPN</a>\n` +
...
const filteredServers = servers.filter(server => {
  ...
  return true;
      ...
      const serverList = currentServers.map(server => {
        ...
        const isFull = (server.total_create_akun || 0) >= server.batas_create_akun;
        ...
        return `🌐 *${server.nama_server}*\n` +
               ...
               (isFull ? `⚠️ *Server Penuh*` : `👥 Total Create Akun: ${server.total_create_akun || 0}/${server.batas_create_akun}`);
      }).join('\n\n');
...
bot.action(/navigate_(\w+)_(\w+)_(\d+)/, async (ctx) => {
  ...
  bot.action(/(renew|create)_username_(ssh)_(.+)/, async (ctx) => {

  if (ctx.answerCbQuery) await ctx.answerCbQuery(); // 🔥 WAJIB
  ...
  db.get(
    ...
    (err, server) => {
      ...
      if (err) {
        return ctx.reply('❌ Error ambil server');
      }
      ...
      if (action === 'create') {
        ...
        state.step = `password_${action}_${type}`;
        return ctx.reply('🔑 Masukkan password renew:');
      } else {
        state.step = `exp_${action}_${type}`;
        return ctx.reply('⏳ Masukkan masa aktif (hari):');
      }
...
// TAMBAH DI SINI 👇
db.run(
  "UPDATE Server SET total_create_akun = COALESCE(total_create_akun, 0) + 1 WHERE id = ?",
  [serverId]
);
...
async function recordAccountTransaction(userId, type) {
  return new Promise((resolve, reject) => {
    ...
    db.run(
      'INSERT INTO transactions (user_id, type, reference_id, time) VALUES (?, ?, ?, ?)',
